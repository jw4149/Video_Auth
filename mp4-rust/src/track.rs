...
impl Mp4Track {
    ...
    fn parse_nalu(stream: &[u8], start_index: usize) -> (usize, usize, usize, usize, usize, usize) {
        if start_index >= stream.len() {
            return (0, 0, stream.len(), 0, 0, 0); // No more data to read
        }
        ...
        let forbidden_bit =  (nalu_header >> 7) & 0x01; // Extract the first bit
        let nal_ref_idc = (nalu_header >> 5) & 0x03;   // Extract the next two bits
        let nal_unit_type = nalu_header & 0x1F;

        let next_nalu_start = nalu_start + nalu_length; // Position of the next NALU
    
        (nalu_length, nalu_header, next_nalu_start, forbidden_bit, nal_ref_idc, nal_unit_type)
    }
    ...
    pub(crate) fn read_sample<R: Read + Seek>(
        ...
        while start_index < buffer.len() {
            let (nalu_length, nalu_header, next_nalu_start, forbidden_bit, nal_ref_idc, nal_unit_type) = Self::parse_nalu(&buffer, start_index);
            if nalu_length == 0 {
                break;
            }
    
            println!("NALU Length: {}, NALU Header: 0x{:02X}, Next NALU Start: {}, forbidden_bit:{}, nal_ref_idc:{}, nal_unit_type:{}", nalu_length, nalu_header, next_nalu_start, forbidden_bit, nal_ref_idc, nal_unit_type);
            start_index = next_nalu_start; // Move to the next NALU
            cnt += 1;
            if cnt> 5 {
                break;
            }
